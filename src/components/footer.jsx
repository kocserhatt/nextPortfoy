export default function Footer(){

    return (
      <>
        <div id="baglantilar" className="footer">
            <span className="spn-3">Bağlantı</span>
            <h1 className="h1-3">Bağlantı kurmak</h1>
            <p className="p-2">kocserhat671@gmail.com</p>
            <div className="icon">
                <a href="https://www.linkedin.com/in/serhat-ko%C3%A7-56b30929b/"><img src="/LinkedIn.svg" alt="" /></a>
                <a href="https://github.com/kocserhatt"><img src="/GitHub.svg" alt="" /></a>
                <a href="https://www.instagram.com/koc.serhatt/"><img src="/Instagram.svg" alt="" /></a>
            </div>
            <p className="footer-end">Copyright © 2024. Develop & Designed by Serhat Koç</p>
        </div>
</>
    )
}